...
export default function WithdrawalPage({ params }: { params: { tenant: string } }) {
  ...
  const { bookings = [], treatments = [], loading: dataLoading } = useAppContext()
  ...
  // For now, assume 20% is available for withdrawal (you can adjust this logic)
  const [balance, setBalance] = useState(0)
  const [totalWithdrawn, setTotalWithdrawn] = useState(0)
  const [withdrawalHistory, setWithdrawalHistory] = useState<WithdrawalHistory[]>([])

  useEffect(() => {
    // Calculate initial balance (80% of total earnings - total withdrawn)
    const availableBalance = totalEarnings * 0.8 - totalWithdrawn
  ...
  const totalPages = Math.ceil(withdrawalHistory.length / pageSize)
  const paginatedHistory = withdrawalHistory.slice((currentPage - 1) * pageSize, currentPage * pageSize)
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between pt-4 border-t">
                  <div className="text-sm text-gray-600">
                    Menampilkan {((currentPage - 1) * pageSize) + 1} - {Math.min(currentPage * pageSize, withdrawalHistory.length)} dari {withdrawalHistory.length} penarikan
                  </div>
