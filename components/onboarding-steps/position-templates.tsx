...
import { Briefcase, Plus, Trash2, Info, CheckCircle2, Sparkles, ChevronDown } from "lucide-react"
import { useOperationalOnboarding } from "@/lib/operational-onboarding-context"
import { motion, AnimatePresence } from "framer-motion"

interface PositionTemplatesStepProps {
...
export function PositionTemplatesStep({ onValidChange }: PositionTemplatesStepProps) {
  const { toast } = useToast()
  const { progress, setStaffPositionTemplates } = useOperationalOnboarding()
  ...
  // Default templates for beauty clinic/salon/spa
  const getDefaultTemplates = (): PositionTemplate[] => [
    { id: "1", name: "Therapist", description: "Beauty therapist/treatment specialist", is_default: true, is_selected: true },
    { id: "2", name: "Beautician", description: "Beauty specialist", is_default: true, is_selected: true },
    { id: "3", name: "Massage Therapist", description: "Massage specialist", is_default: true, is_selected: true },
    { id: "4", name: "Nail Technician", description: "Nail care specialist", is_default: true, is_selected: true },
    { id: "5", name: "Hair Stylist", description: "Hair treatment and styling", is_default: true, is_selected: true },
    { id: "6", name: "Receptionist", description: "Front desk and customer service", is_default: true, is_selected: true },
  ]
  ...
  useEffect(() => {
    if (!loading) {
      ...
      setStaffPositionTemplates(selectedTemplates)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const addCustomPosition = () => {
    ...
    const exists = templates.some(t =>
      t.name.toLowerCase() === customPosition.trim().toLowerCase()
    )

    if (exists) {
      toast({
        title: "Sudah Ada",
        description: "Posisi ini sudah ada dalam daftar",
        variant: "destructive",
      })
      return
    }

    const newTemplate: PositionTemplate = {
      id: `custom-${Date.now()}`,
    ...
    toast({
      title: "Berhasil",
      description: "Posisi kustom berhasil ditambahkan",
    })
  }
  ...
  const removeCustomPosition = (id: string) => {
    ...
    toast({
      title: "Dihapus",
      description: "Posisi kustom berhasil dihapus",
    })
  }
  ...
  const handleUseDefaults = () => {
    ...
    toast({
      ...
      description: "Semua posisi default telah dipilih",
    })
  }
    ...
    <div className="space-y-6 max-w-full overflow-x-hidden">
          ...
          <div className="flex items-start gap-3 min-w-0 w-full">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-blue-700 leading-relaxed break-words">
                Pilih posisi staff yang akan digunakan di bisnis Anda. Anda bisa skip dan menggunakan default, atau customize sesuai kebutuhan.
              </p>
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-start gap-3 min-w-0 w-full">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-purple-700 leading-relaxed break-words">
                Posisi yang jelas membantu mengorganisir staff, menetapkan skill/keahlian, dan mempermudah penjadwalan layanan.
              </p>
            </div>
          </div>
        </Card>
      ...
      <Alert className="border-green-200 bg-green-50">
        ...
        <AlertDescription className="text-green-800 flex items-center justify-between">
          <span>
            {selectedCount} posisi terpilih. Klik tombol di bawah untuk menggunakan semua default dan lanjut.
          </span>
          <Button
            variant="outline"
            size="sm"
        ...
        <Card className="overflow-hidden rounded-xl max-w-full">
          ...
          >
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <h3 className="font-semibold text-gray-900 truncate">Pilih Posisi Staff</h3>
                      ...
                      >
                        ...
                        {!template.is_default && (
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => removeCustomPosition(template.id)}
      ...
      <Card className="p-6 rounded-xl max-w-full overflow-x-hidden">
        <div className="flex items-center gap-3 mb-4 min-w-0">
          ...
          <h3 className="text-base font-semibold truncate">Tambah Posisi Kustom</h3>
        </div>

        <div className="flex gap-3">
          <div className="flex-1">
            <Input
              placeholder="Contoh: Senior Therapist, Manager"
